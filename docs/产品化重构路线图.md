...
- 2026-06-18：推进 Phase 3.2 / Phase 4.1 小切片：PDF 导入后自动尝试外部元数据反查，统一接入 Semantic Scholar、OpenAlex 与 arXiv 候选；只有标题强匹配时才把引用数、分类、DOI/arXiv、外链等字段合并到用户上传论文，保留 `paper_source=user_upload`，并通过 `metadata_enrichment_status` 标注 matched/unmatched/failed，避免错配论文污染证据池；研究上下文读取 `taxonomy_category/category` 双字段，确保上传 PDF 的分类和引用能传到 workspace。
- 2026-06-18：推进 Phase 6.1 / Phase 3.3 小切片：新增 Paper Brief LLM 编辑层，只允许基于已有标题、摘要、正文片段、入选信号和 claim checks 改写论文阅读建议，不允许新增证据或暴露内部 token；产品运行时在有 API key 的情况下默认 auto-on，pytest 默认 offline；每个 workspace 默认只增强前 4 篇高优先级论文（可用 `PAPER_BRIEF_LLM_MAX_PER_WORKSPACE` 调整），workspace 论文卡片构建新增可控并行开关（`ENABLE_WORKSPACE_PAPER_PARALLEL=1` / `WORKSPACE_PAPER_BRIEF_WORKERS`），为后续 Parallel Paper Analysts 做基础铺垫。
- 2026-06-18：继续推进 Phase 3.3 论文详情后端增强：`claim_verification_service` 新增章节/页码感知的全文候选 claim 抽取，优先从 Method、Experiments/Results、Contribution、Limitation 等段落抽取可核验主张；workspace `claim_checks` 新增 `page` 与 `confidence` 字段，并修正 Paper Brief 生成链路中“先清洗压扁 review_text 导致页码/章节丢失”的问题。后续验收重点：用户上传 PDF 的论文详情应能显示更具体的正文片段证据，而不是只停留在摘要级或泛化模板。
- 2026-06-18：继续推进 Phase 3.2 / Phase 3.3 论文详情可读性增强：Evidence 论文详情新增“阅读决策、适合支撑、读的时候问、不要误用”四格决策面板，把每篇论文从“元数据+摘要”提升为“这篇怎么用”的阅读助手；主张核验区改为中文化的 claim type、核验状态汇总和证据层级说明，旧 Paper Brief 继续折叠为结构化摘要。后续验收重点：论文详情是否能帮助用户快速判断先读哪篇、为什么读、哪些结论还能不能信，而不是只展示字段。
- 2026-06-18：继续推进 Phase 5.2.3 / Phase 3.2 / Phase 4.2 小切片：后端将 `team_process`、`topic_match` 等 snake_case 内部匹配信号统一视为不可展示理由，避免污染“做了什么 / 为什么先读 / Paper Brief”；通信-计算编排、端云协同、资源调度类论文新增专门的 Paper Brief 标签、方法解释和阅读重点；前端将旧 Paper Brief 降级为可展开“结构化摘要”，右侧结果区在首次生成和每轮追问后都稳定给出可继续追问建议；演进图节点大小开始融合引用数与连接度，边从节点边界出发并加入曲率偏移，降低节点/边重叠观感。后续验收需要重点看：旧任务快照是否仍有内部字段泄露、追问建议是否真的帮助下一轮提问、图谱大小编码是否会误导用户把高引用等同于强相关。
...
- 2026-06-18：已推进 Phase 2.3 研究简报可信叙事，后端 `research_brief` 会优先说明当前主题的路线覆盖、核心论文来源与选择依据、空白/建议的证据等级和下一步动作；无稳定建议时也会给出可执行阅读/补搜动作，避免退化成计数摘要或空态。
- 2026-06-18：继续推进 Phase 2.3 / Phase 3.2，`research_brief` 新增领域格局、证据选择依据和决策建议字段；`paper_brief` 新增为什么选中、建议读法、证据依据和验证边界，并从摘要/正文片段中补充方法、评测、局限等 claim 候选。前端 Overview 与 Evidence 已优先展示这些结构化字段。
- 2026-06-18：推进 Phase 3.3 第一版全文级 claim verification：对用户上传/导入 PDF 抽取出的 `review_text` 做页码/章节感知证据定位，将 Paper Brief claim check 从摘要级升级为正文片段级 `verified / partial`；同时增加轻量 LLM 研究简报压缩层，只改写 `headline / executive_summary / landscape_overview / evidence_rationale / decision_advice` 等展示文本，不改变论文 ID、证据绑定和结论准入。
...
| 场景 | 用户痛点 | Product Agent 的价值 |
| --- | --- | --- |
| 新进入一个研究方向 | 不知道该搜什么、先读哪几篇、领域主线是什么 | 自动拆查询、检索论文、生成领域格局和优先阅读入口 |
| 已有几篇本地 PDF | 不想从零问 AI，也不想手动整理每篇贡献 | 将用户上传论文纳入证据池，生成 Paper Brief 和正文片段级 claim checks |
| 多轮追问深化选题 | 普通 AI 容易忘记历史、只给泛泛建议 | 保留工作台、working memory、沿用/新增论文和研究总览 |
| 准备开题/小论文选题 | 需要知道空白和建议是否真有论文支撑 | 对 gap / idea 标注直接、间接或待验证证据等级，并展示支撑论文 |
| 答辩或组会展示调研过程 | 需要解释系统如何得出结论，而不只是结果 | Run 区展示 Agent 流程，Evidence/Map/Context 可追溯证据与上下文边界 |

与“直接问 AI”的差异：

- 直接问 AI 更擅长快速解释概念；Product Agent 更强调“论文证据池 + 结构化工作台 + 多轮沉淀”。
- 直接问 AI 往往难以说明论文怎么选；Product Agent 会给出核心论文选择依据、来源、引用状态、是否本轮新增。
- 直接问 AI 的建议可能没有证据边界；Product Agent 要把建议分成直接证据、间接支撑和待验证假设。
- 直接问 AI 难以利用用户自己的 PDF；Product Agent 可以把用户上传论文作为本研究证据，并定位正文片段。
...
- 已完成第一版正文片段级核查：针对 `review_text` 中可解析的 `[Page N]`、章节标题和句子，定位 claim 的支持片段，并写入 `claim_checks` 的 `source_level / section / evidence / caveat`。
- 已支持 `verified / partial / unknown` 的展示语义，但 `verified` 仍表示“可解析正文片段支持”，不是完整自动审稿。
- 下一步如果继续增强，应把 PDF 正文解析结果持久化为 section/chunk artifact，并让 Paper Analyst 并行处理核心论文前 4-8 篇。
...
- 研究简报进展概括已完成后端第一版：`executive_summary` 不再优先复述“论文 N 篇”，而是从 taxonomy 路线、核心论文来源、直接相关论文、用户上传/系统检索占比、空白和建议证据等级组织叙事。
- “建议先读几篇核心论文”的理由已加入选择依据：来源、直接相关、本轮新增、分支覆盖、高引用、正文片段等因素。
- Overview 已开始直接展示领域格局、核心论文选择依据和下一步决策建议；Evidence 论文详情已开始展示为什么选中、建议读法、证据依据、验证边界和 claim checks。
- 研究简报已加入轻量 LLM 压缩层：只作为保守编辑器改写展示文本，不改变证据绑定；无 API key、环境变量跳过或模型返回无效时自动使用 deterministic brief。
- 后续仍需继续做前端视觉验收：确认 Overview 中 brief 不截断，必读论文和证据等级解释在桌面缩放下可读。

仍缺的关键能力：

- 全文级 claim verification 已有正文片段级第一版，但还没有完整的 PDF section artifact、冲突检测和实验数值复核。
- LLM 研究简报压缩已接入，但当前只做展示文本编辑；后续可把多轮观点压缩和 stable finding 合并做得更细。
- 检索质量仍需要小而硬回归集持续约束，尤其是新主题、追问补搜、用户 PDF + 系统检索混合场景。
- Context 区已经做过叙事化第一版，但共享知识如何影响 query、如何被排除为论文证据，后端仍缺更结构化的 `usage_reason / influence_type`。

## 2026-06-18 Searcher Parallel Retrieval Update

- Phase 6.1 has advanced one more step: the Searcher first-pass retrieval fan-out can now run arXiv and Semantic Scholar calls across multiple queries in parallel.
- Runtime controls: `ENABLE_SEARCHER_PARALLEL=1` forces the feature on, `DISABLE_SEARCHER_PARALLEL=1` turns it off, and `SEARCHER_PARALLEL_WORKERS` controls the worker cap with an internal maximum of 8.
- The retrieval outcome now records `retrieval_parallelism` so workspace/debug views can explain whether first-pass retrieval ran in parallel, how many tasks were submitted, and how many workers were used.
- Scope boundary: recall rescue, facet rescue, taxonomy, evolution graph, auditor, corrector, synthesizer, and working-memory updates remain serial for now. This keeps final reasoning deterministic while speeding up external I/O.
