...
class ConversationService:
    ...
    def create_conversation(self, *, topic: str, title: str | None = None) -> Conversation:
        ...
        参数:
        - topic: 研究主题，是整轮会话的默认主题
        - title: 可选标题；为空时回退为 topic

        返回:
        - Conversation: 已分配 `conversation_id` 的会话对象

        副作用:
        - 调用 repository.create 写入会话
        ...
        错误策略:
        - 当前版本默认假定上层已完成 topic 基础校验
        - 后续可以在此补业务校验异常

        TODO(iter3-conversation-service):
    ...
    def get_conversation(self, conversation_id: str) -> Conversation | None:
        ...
        - 后续的 MessageService / ContextService

        参数:
        - conversation_id: 会话唯一标识

        返回:
        - Conversation | None: 找到则返回对象，否则返回 None
        """
