...
`研究主题 -> 首轮生成 -> 本研究总览 -> 继续追问 -> 本次结果 -> 工作记忆与知识证据沉淀`

## 当前进度校准

这部分用于和迭代三 PDF 要求对齐，方便组员接手。

- 已成型：连续研究工作台主链路、前后端启动、SQLite 持久化、会话 / 消息 / 任务 / workspace、历史记录、工具启停、知识导入、论文候选导入、知识范围选择、working memory、知识命中和证据展示。
- 基本达标：用户管理对话、用户管理工具、跨对话知识共享、多轮追问上下文理解、RAG 知识增强雏形。
- 尚未达标：用户添加 skill。当前只有 `GET /skills` 只读展示，不能由用户新增、启停、校验依赖或在任务中选择 skill。
- 交接重点：请优先看 [任务清单](D:/product_agent/docs/任务清单.md) 和 [前后端分工说明](D:/product_agent/docs/前后端分工说明.md) 中的 skill 添加任务拆解。
