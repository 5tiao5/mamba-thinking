  ...
  gaps: [
      {
        ...
        supporting_paper_ids: ["paper-toolbench", "paper-agentbench"],
        evidence_level: "indirect",
        evidence_reason: "现有评测论文覆盖了工具调用能力，但对失败诊断与恢复路径的刻画仍不充分。",
      },
      {
        summary: "跨工具依赖和中间状态污染尚未被系统度量",
        severity: "medium",
        evidence: ["多步任务中工具输出会影响后续决策", "现有任务常把工具调用看作独立动作"],
        supporting_paper_ids: ["paper-toolbench", "paper-webarena"],
        evidence_level: "indirect",
        evidence_reason: "多步工具任务提供了相关观察，但还缺少针对状态污染的直接实验。",
      },
      {
        ...
        evidence_reason: "当前核心论文尚未提供稳定的成本与延迟对比数据，需要补充专项证据。",
      },
  ],
  ideas: [
    {
      ...
      motivation: "真实工具调用经常失败，现有 benchmark 不足以评估 agent 是否能诊断并恢复。",
        ...
        supporting_paper_ids: ["paper-toolbench", "paper-agentbench"],
        evidence_level: "indirect",
        evidence_reason: "建议由现有工具使用评测框架延伸而来，但错误注入与恢复指标仍需实验验证。",
        raw_text: "",
      },
    {
      ...
      motivation: "生产环境中工具调用成本和延迟会影响可用性。",
        ...
        supporting_paper_ids: [],
        evidence_level: "exploratory",
        evidence_reason: "当前论文池没有直接验证预算约束策略，这是一条待验证的产品研究方向。",
        raw_text: "",
      },
  ],
  alignment_score: 0.82,
